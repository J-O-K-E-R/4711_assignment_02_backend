...
class Recipes extends CI_Model {
    ...
    public function getIngredients($recipeID){
        $sql = sprintf("SELECT * from SUPPLIES inner join RECIPESUPPLIES on SUPPLIES.id = RECIPESUPPLIES.supplyID inner join RECIPES on RECIPESUPPLIES.recipeID = RECIPES.ID where recipeID = %d", $recipeID);
        $query = $this->db->query($sql);
        return $query->result();
    }
